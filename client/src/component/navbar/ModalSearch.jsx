...
const ModalSearch = ({closeMenu}) => {
    ...
    const handleSubmit =async()=>{
      try {
        const response =await axios.get(`https://blog-website-lbk2.onrender.com/blog/search?search=${search}`)
    ...
    <div className='fixed top-0 left-0 flex flex-row justify-center items-center w-full h-full bg-gray-900 opacity-80'>
        <form action="" method="post" onSubmit={searchButtonClicked} >
            <div className='flex justify-between items-center py-3'>
                <h2 className='text-white text-2xl '>Search here</h2>
                <div onClick={closeMenu}> <i className="fas fa-times text-white text-3xl"></i></div>
            </div>
            <div className=' '> 
                ...
                <div className='flex justify-center py-2 border border-solid bg-white text-black shadow-lg my-3 rounded-lg h-40'>
        <ul className=' '>
        {searchResult && searchResult.map(item => {
        return  <li key={item.id} className='text-lg text-black'><Link to={`/blog/single/${item.id}`}>{item.title}</Link></li>
})}
